 ...
 */
	function core_theme_hide_show_bg(){
		echo '<div id="hide-show-bg" class="hide-phone"><i class="icon-minus"></i> <i class="icon-plus"></i> <i id="sliderPrev" class="icon-chevron-left "></i> <i id="sliderPlayPause" class="icon-pause"></i> <i id="sliderNext" class="icon-chevron-right"></i><span class="hideshow-name">ThemeDutch</span></div>';
 ...
 */
	function core_theme_social_icons(){
		echo '<div class="social-blk right">';
		core_sociables('social_icons');
		echo '</div>';
	} // core_theme_social_icons()
}
add_action('core_theme_hook_before_header', 'core_theme_social_icons',20);
 ...
 */
	...
	?>
	<div class="logo-blk left <?php echo core_options_get('logo_align'); ?>">
...
add_action('core_theme_hook_before_header', 'core_theme_logo',1);
 ...
 */
	function core_theme_top_menu(){
		echo '<nav id="site-navigation" class="grid fit">';
...
if ( ! function_exists( 'core_theme_bgslider' ) ) {
/**
 * Background Slider Area
 *
 * @since framework 1.0
 */
	function core_theme_bgslider(){

		$bgsetup = core_options_get('background_setup');
		$slider = core_options_get('background_slider');
...
add_action('core_theme_hook_footer_after', 'core_theme_bgslider');
 ...
 */
	function core_theme_footer_menu(){
		//if( core_layout_menu('footer') ){
			echo '<nav class="footer-menu left">';
 ...
 */
	function core_theme_copyright(){
		echo '<div id="theme-copyright" class="footer-text right">';
 ...
 */
 	function core_theme_meta_icons(){
	 	//if ( is_single() ) {
			...
			echo '<div class="grid box-six fit text-right float-right"><ul class="entry-meta-list">';
 ...
 */
	function core_theme_posted_on() {
		...
		if ( !is_page() && $meta && is_single() ) {
			...
			printf( __( '<div class="posted-on-pnl">Posted on: <span class="date"> <a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span> ', THEME_SLUG ),
